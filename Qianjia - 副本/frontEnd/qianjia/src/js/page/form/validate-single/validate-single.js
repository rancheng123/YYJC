            ...
            if(rules){
                ...
                for(var rule of rules){
                    ...
                    if(validate_rules.hasOwnProperty(ruleName)){
                        ...
                        validate_rules[ruleName].reg(value,reactEle,ruleParams,that,currentRuleName,function(res){
                            that.count++;

                            that.validateCircle[res.currentRuleName] = true;

                            if(!res.isBool){
                                //单个元素多个错误，只报一个错误
                                if(that.isOK){
                                    that.showError({
                                        errorMsg: msgPrefix + validate_rules[ruleName].messages(value,reactEle,ruleParams,that,res.code),
                                        errorRule:ruleName,
                                        nativeEle,
                                        reactEle
                                    })
                            ...
                            if(that.isValidateCompelete()){
                                console.log(ruleName,that.isOK);
